...
async function getGithubUrl(npmUrl: string): Promise<string> {
  ...
  const githubUrlWithPackageName =
    githubUrl.split("/")[0] + "/" + githubUrl.split("/")[1] + "/" + packageName;
      ...
      for (let url of urls) {
        ...
        const netScore =
          ...
          (responsiveScore * 0.3 +
            ...
            rampUpScore * 0.15);
        console.log(
          `{"URL":"${newUrl}", "NET_SCORE":${formatter(
            netScore,
          )}, "RAMP_UP_SCORE":${formatter(
            rampUpScore,
          )}, "CORRECTNESS_SCORE":${formatter(
            correctnessScore,
          )}, "BUS_FACTOR_SCORE":${formatter(
            busFactor,
          )}, "RESPONSIVE_MAINTAINER_SCORE":${formatter(
            responsiveScore,
          )}, "LICENSE_SCORE":${formatter(licenseScore)}}`,
        );
      }
    } catch (error) {
