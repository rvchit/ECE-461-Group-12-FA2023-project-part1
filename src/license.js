        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
                        logger.error("Failed to fetch readme from ".concat(apiURL, ". Status: ").concat(response.statusText));
                        throw new Error("Failed to fetch readme from ".concat(apiURL, ". Status: ").concat(response.statusText));
                    }
                    return [4 /*yield*/, response.json()];
                case 2:
                    content = (_a.sent()).content;
                    readme = Buffer.from(content, 'base64').toString('utf-8');
                    licenseRegex = /licen[sc]e/gi;
                    hasLicense = licenseRegex.test(readme);
                    return [2 /*return*/, hasLicense ? 1 : 0];
            }
        });
    });
}
